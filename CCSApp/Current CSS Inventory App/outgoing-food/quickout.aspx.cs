    ...
    private void LoadGrid()
    {
        using (CCSEntities db = new CCSEntities())
        {
            grdFoodOut.DataSource = (from f in foodOutRecords
                                    select new { id = f.DistributionID, category = (f.FoodCategory == null ? f.USDACategory.Description : f.FoodCategory.CategoryType), weight = f.Weight, cases = f.Count }).ToList();
