...
public partial class Default2 : System.Web.UI.Page
{
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            double num;
            int iNum;
            Boolean needCaseCount = isCaseCountNeeded();
            ...
            else if (needCaseCount && !int.TryParse(txtNumberOfCases.Text, out iNum)) //checks that it is a number OR it isn't used
            {
                lblWeightError.Text = "Invalid number of cases!";
            }
            else if (needCaseCount && int.Parse(txtNumberOfCases.Text) < 0)  //checks that number is not less than 0 OR it isn't used
            {
                lblWeightError.Text = "Invalid number of cases!";
            }
            else if (ddlFoodSourceType.SelectedValue.Equals("0"))
            {
                lblWeightError.Text = "You must select a Food Source Type.";
            }
            else
            {
                using (CCSEntities db = new CCSEntities())
                {
                    ...
                    if (!chkIsUSDA.Checked)
                    {
                        ...
                        if (fc.CaseWeight != null) // if case weight is used, calculate the weight
                        {
    ...
    private Boolean isCaseCountNeeded()
    {
        ...
        try
        {
            using (CCSEntities db = new CCSEntities())
            {
                if (!chkIsUSDA.Checked)
                {
                    ...
                    if (fc.CaseWeight != null) // if case weight is used, case count is needed
                        result = true;
                }
                else // if it is USDA, then case count is needed
                    result = true;
            }
        }
        catch (System.Threading.ThreadAbortException)
