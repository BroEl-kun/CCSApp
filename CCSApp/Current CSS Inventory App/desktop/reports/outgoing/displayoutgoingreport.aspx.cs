...
public partial class desktop_reports_outgoing_displayoutgoingreport : System.Web.UI.Page
{
    ...
    private ReportsDataSet LoadData(DateTime startdate, DateTime enddate, OutgoingReportTemplate template)
    {
        ...
        {
            List<FoodOut> data = db.FoodOuts.Where(f => (f.TimeStamp.Year >= startdate.Year && f.TimeStamp.Day >= startdate.Day && f.TimeStamp.Month >= startdate.Month) &&
            ...
            foreach (var i in data)
            {
                if(i.FoodCategory != null || i.USDACategory != null)
                    ds.Outgoing.AddOutgoingRow(i.FoodCategory == null? i.USDACategory.Description: i.FoodCategory.CategoryType, i.TimeStamp, (double)(i.Count ?? 0), i.Weight, i.Agency == null ? "No-Agency" : i.Agency.AgencyName, i.DistributionType.DistributionType1, i.FoodSourceType.FoodSourceType1);
            }
        }
        return ds;
    }
