            ...
            if (template.CategoriesSelection != ReportTemplate.SelectionType.NONE)
            {
                foreach (var f in foodCategoriesData)
                {
                    ...
                    double weightIn = (from w in f.FoodIns
                                       where (w.TimeStamp.Year >= startdate.Year && w.TimeStamp.Day >= startdate.Day && w.TimeStamp.Month >= startdate.Month) &&
                                            (w.TimeStamp.Year <= enddate.Year && w.TimeStamp.Day <= enddate.Day && w.TimeStamp.Month <= enddate.Month)
